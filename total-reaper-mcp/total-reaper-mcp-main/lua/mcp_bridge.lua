...
local function GetTimeSignature()
    -- TimeMap_GetTimeSigAtTime returns: timesig_num, timesig_denom, tempo (3 values, no retval)
    local num, denom, tempo = reaper.TimeMap_GetTimeSigAtTime(0, reaper.GetCursorPosition())
    return {ok = true, numerator = num, denominator = denom, tempo = tempo}
end

local function SetTimeSignature(numerator, denominator)
    -- Get current tempo to preserve it
    local tempo = reaper.Master_GetTempo()
    -- Add a tempo/time sig marker at position 0 with new time signature
    reaper.SetTempoTimeSigMarker(0, -1, 0, -1, -1, tempo, numerator, denominator, true)
                        ...
                        if #args >= 3 then
                            ...
                            if track then
                                ...
                                if fx_index >= 0 then
                                    ...
                                    response.ok = false
                                    response.ret = -1
                                    response.error = "FX not found: " .. tostring(args[2])
                                end
                            else
                                response.error = "Track not found"
                                response.ok = false
