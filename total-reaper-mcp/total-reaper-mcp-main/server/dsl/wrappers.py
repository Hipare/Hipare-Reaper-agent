...
async def transport_set_tempo(bridge, bpm: float) -> OperationResult:
    ...
    try:
        ...
        # Get current time signature to preserve it
        direct_result = await bridge.call_lua("TimeMap_GetTimeSigAtTime", [0, 0.0])
        numerator = int(direct_result.get("ret", 4)) if direct_result.get("ok") else 4
        dsl_result = await bridge.call_lua("GetTimeSignature", [])
        if dsl_result.get("ok"):
            dsl_denom = dsl_result.get("denominator", 4)
            if dsl_denom in [1, 2, 4, 8, 16, 32]:
                denominator = int(dsl_denom)
            else:
                dsl_num_as_denom = dsl_result.get("numerator", 4)
                denominator = int(dsl_num_as_denom) if dsl_num_as_denom in [1, 2, 4, 8, 16, 32] else 4
        else:
            denominator = 4

        # Delete existing tempo markers and create fresh one with new tempo
        ...
        for i in range(marker_count - 1, -1, -1):
            await bridge.call_lua("DeleteTempoTimeSigMarker", [0, i])

        # Create marker at position 0 with new tempo, preserving time signature
        result = await bridge.call_lua("SetTempoTimeSigMarker",
                                        [0, -1, 0.0, -1, -1, bpm, numerator, denominator, True])
        if not result.get("ok"):
...
async def transport_set_time_signature(bridge, numerator: int, denominator: int) -> OperationResult:
    """Set project time signature"""
    try:
        # Validate inputs
        if numerator < 1 or numerator > 32:
            raise Exception(f"Invalid numerator: {numerator}. Must be 1-32.")
        if denominator not in [1, 2, 4, 8, 16, 32]:
            raise Exception(f"Invalid denominator: {denominator}. Must be 1, 2, 4, 8, 16, or 32.")

        # First get current tempo to preserve it
        tempo_result = await bridge.call_lua("Master_GetTempo", [])
        current_tempo = tempo_result.get("ret", 120.0) if tempo_result.get("ok") else 120.0

        # Count existing markers for debugging
        count_result = await bridge.call_lua("CountTempoTimeSigMarkers", [0])
        marker_count = count_result.get("ret", 0) if count_result.get("ok") else 0

        # Delete ALL existing tempo markers to start clean
        for i in range(marker_count - 1, -1, -1):
            await bridge.call_lua("DeleteTempoTimeSigMarker", [0, i])

        # Create a fresh tempo/time sig marker at position 0
        result = await bridge.call_lua("SetTempoTimeSigMarker",
                                        [0, -1, 0.0, -1, -1, current_tempo, numerator, denominator, True])
        ...
        verify_result = await bridge.call_lua("TimeMap_GetTimeSigAtTime", [0, 0.0])
        verify_num = verify_result.get("ret", "?") if verify_result.get("ok") else "?"
        logger.info(f"SetTimeSig: markers_before={marker_count}, set={numerator}/{denominator}, verify_raw={verify_result}")

        return OperationResult(
            success=True,
            action="transport_set_time_signature",
            message=f"Set time signature to {numerator}/{denominator}",
            changes={
                "numerator": numerator,
        ...
        return OperationResult(
            success=False,
            action="transport_set_time_signature",
            message="Failed to set time signature",
            error=str(e)
        )
...
async def context_get_tempo_info(bridge) -> OperationResult:
    ...
    try:
        ...
        # Get time signature using two methods to handle Lua bug:
        # 1. Direct REAPER API call: generic bridge returns first return value = timesig_num
        direct_result = await bridge.call_lua("TimeMap_GetTimeSigAtTime", [0, 0.0])
        # 2. DSL GetTimeSignature function (may have old or new Lua code)
        dsl_result = await bridge.call_lua("GetTimeSignature", [])

        if direct_result.get("ok") and dsl_result.get("ok"):
            # Direct API call's "ret" = first return = timesig_num (always correct)
            numerator = int(direct_result.get("ret", 4))

            # DSL function: if Lua is fixed, "denominator" has correct value
            # If Lua is buggy (old code), "numerator" field actually has the real denominator
            dsl_denom = dsl_result.get("denominator", 4)
            if dsl_denom in [1, 2, 4, 8, 16, 32]:
                denominator = int(dsl_denom)
            else:
                # Old Lua bug: "numerator" field = real denominator, "denominator" = tempo
                dsl_num_as_denom = dsl_result.get("numerator", 4)
                if dsl_num_as_denom in [1, 2, 4, 8, 16, 32]:
                    denominator = int(dsl_num_as_denom)
                else:
                    denominator = 4  # Ultimate fallback
        elif dsl_result.get("ok"):
            numerator = dsl_result.get("numerator", 4)
            denominator = dsl_result.get("denominator", 4)
            if denominator not in [1, 2, 4, 8, 16, 32]:
                denominator = 4
        else:
            numerator = 4
