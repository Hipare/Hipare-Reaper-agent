...
def _get_vst_display_names() -> Dict[str, str]:
    """Read reaper-vstplugins64.ini and build a mapping from short names to REAPER display names.

    Returns dict like: {'emissary': 'Ignite - Emissary (STL Tones)', 'nadir': 'Ignite - NadIR (STL Tones)'}
    """
    try:
        # Derive REAPER path from bridge dir (bridge_dir is .../REAPER/Scripts/mcp_bridge_data)
        bridge_dir = os.environ.get('REAPER_MCP_BRIDGE_DIR', '')
        if bridge_dir:
            reaper_path = Path(bridge_dir).parent.parent
        else:
            # Fallback: common Windows path
            reaper_path = Path(os.path.expanduser('~')) / 'AppData' / 'Roaming' / 'REAPER'

        ini_path = reaper_path / 'reaper-vstplugins64.ini'
        ...
        with open(ini_path, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                ...
                if not line or line.startswith('['):
                    continue
                # Format: PluginName.dll=hash,id,Display Name (Manufacturer)
                # or: PluginName.vst3=hash,id{uid,Display Name (Manufacturer)
                if '=' not in line:
                    continue
                file_part, _, rest = line.partition('=')
                # Extract DLL/VST3 base name
                ...
                # Extract display name (everything after second comma)
                # Format: hash,id,Display Name (Manufacturer) - name may contain commas
                # VSTi plugins have !!!VSTi suffix, strip it
                parts = rest.split(',', 2)
                if len(parts) >= 3:
                    display_name = parts[2].strip()
                    # Strip REAPER flags like !!!VSTi, !!!VSTiM etc.
                    if '!!!' in display_name:
                        display_name = display_name[:display_name.index('!!!')].strip()

                    if display_name:
                        key = base_name.lower()
                        # Only store the first (VST2) entry, prefer it over VST3
...
# Cache VST display names (loaded once)
_vst_display_name_cache = None

def _get_cached_vst_names() -> Dict[str, str]:
    """Get cached VST display names, loading if needed."""
    global _vst_display_name_cache
    if _vst_display_name_cache is None:
        _vst_display_name_cache = _get_vst_display_names()
    return _vst_display_name_cache

def _refresh_vst_cache():
    """Force refresh of VST name cache."""
    global _vst_display_name_cache
    _vst_display_name_cache = _get_vst_display_names()
from .wrappers import (
    ...
    transport_play, transport_stop, transport_set_tempo, transport_set_time_signature,
...
def register_dsl_tools(mcp):
    ...
    @mcp.tool()
    async def dsl_set_time_signature(numerator: int, denominator: int) -> str:
        """
        Change the time signature of your project. Use for meter changes.

        Args:
            numerator: Beats per bar (e.g., 3, 4, 5, 6, 7)
            denominator: Note value per beat (e.g., 4 for quarter note, 8 for eighth note)

        Examples:
            - "change to 3/4 time"
            - "set time signature to 6/8"
            - "make it waltz time" (3/4)
            - "switch to 7/8"
        """
        result = await transport_set_time_signature(bridge, numerator, denominator)
    ...
    async def dsl_add_effect(
        ...
        try:
            ...
            # Add the effect - try multiple name variations for third-party plugins
            try:
                result = await track_fx_add_by_name(track_index, fx_name)
            except Exception:
                result = f"Failed to add {fx_name}"

            # If failed, try using the REAPER display name from the VST ini file
            if "Failed" in result:
                vst_names = _get_cached_vst_names()
                search_key = effect.lower().strip()

                # Build list of alternative names to try
                alt_names = []

                def _add_display_name_variants(display_name, alt_names):
                    """Add display name and stripped variants to alt_names list."""
                    import re
                    # Strip all parenthetical suffixes (manufacturer, output config)
                    # e.g. "Sean Pandy Drums (Rob Chokehold) (32 out)" → "Sean Pandy Drums"
                    stripped = re.sub(r'\s*\([^)]*\)', '', display_name).strip()
                    # Also strip just the last parenthetical
                    stripped_last = re.sub(r'\s*\([^)]*\)\s*$', '', display_name).strip()

                    # Try all name variations - order matters (simplest first)
                    for name in [stripped, stripped_last, display_name]:
                        alt_names.extend([
                            f"VST: {name}",
                            f"VST3: {name}",
                            f"VSTi: {name}",
                ...
                # 1. Look up the exact display name from the ini file
                if search_key in vst_names:
                    _add_display_name_variants(vst_names[search_key], alt_names)
                else:
                    # Try partial matching in ini file keys
                    for vst_key, display_name in vst_names.items():
                        if search_key in vst_key or vst_key in search_key:
                            _add_display_name_variants(display_name, alt_names)
                            break

                # 2. Also try with fx_name (might differ from effect if mapped)
                if fx_name.lower() != search_key and fx_name.lower() in vst_names:
                    _add_display_name_variants(vst_names[fx_name.lower()], alt_names)

                # 3. Generic prefix variations as last resort
                alt_names.extend([
                    f"VST: {fx_name}",
                    f"VST3: {fx_name}",
                    f"VST: {effect}",
                ...
                for name in alt_names:
                    if name not in seen:
                        seen.add(name)
                        unique_alt.append(name)

                for alt_name in unique_alt:
                    try:
                        alt_result = await track_fx_add_by_name(track_index, alt_name)
                        if "Failed" not in alt_result:
                            result = alt_result
                            fx_name = alt_name
                            break
                    except Exception:
                        # Bridge error for this name variation, try next
                        continue

            # Check if the effect was actually added after all attempts
            if "Failed" in result:
                # Refresh cache in case new plugins were installed
                _refresh_vst_cache()
                return f"Failed to add {effect} ({fx_name}) to track {track_index + 1}: FX not found in REAPER"
    ...
    async def dsl_adjust_effect(
        ...
        try:
            from .resolvers import resolve_track
            from server.tools.fx import (
                track_fx_get_count, track_fx_get_name, track_fx_get_param_count,
                track_fx_get_param_name, track_fx_set_param, track_fx_get_param
            )
            ...
            if isinstance(value, str):
                ...
                numeric_value = value_map.get(value.lower(), None)
            ...
            # Map effect name (same mapping as add_effect)
            effect_name_map = {
                'reverb': 'ReaVerbate', 'verb': 'ReaVerbate',
                'compression': 'ReaComp', 'compressor': 'ReaComp', 'comp': 'ReaComp',
                'eq': 'ReaEQ', 'equalizer': 'ReaEQ',
                'delay': 'ReaDelay', 'limiter': 'ReaLimit',
                'gate': 'ReaGate', 'noise gate': 'ReaGate',
                'tuner': 'ReaTune', 'viritin': 'ReaTune',
            }
            fx_search_name = effect_name_map.get(effect.lower(), effect).lower()

            # Find the FX by name in track's FX chain
            fx_count_str = await track_fx_get_count(track_index)
            try:
                parts = str(fx_count_str).split()
                if "has" in parts:
            ...
            for i in range(fx_count):
                try:
                    name_result = await track_fx_get_name(track_index, i)
                    # Parse "FX X on track Y: FxName" format
                    fx_name_str = name_result.split(": ", 1)[-1].lower() if ": " in name_result else name_result.lower()
                    if fx_search_name in fx_name_str or fx_name_str in fx_search_name:
                        target_fx_index = i
                        break
                except:
                    continue

            if target_fx_index == -1:
                return f"Effect '{effect}' not found on track {track_index + 1}"

            # Find matching parameter by name
            try:
                param_count_str = await track_fx_get_param_count(track_index, target_fx_index)
                param_count = int(str(param_count_str).split()[-2]) if "has" in str(param_count_str) else 0
            except:
                param_count = 0

            if param_count == 0:
                return f"No parameters found for {effect} on track {track_index + 1}"

            # Setting name aliases for common parameters
            setting_aliases = {
                'ratio': ['ratio', 'comp ratio'],
                'threshold': ['threshold', 'thresh'],
                'attack': ['attack', 'attack time', 'attack ms'],
                'release': ['release', 'release time', 'release ms'],
                'gain': ['gain', 'output gain', 'makeup', 'makeup gain', 'volume'],
                'mix': ['mix', 'wet', 'dry/wet', 'wet/dry', 'drywet'],
                'frequency': ['frequency', 'freq', 'center freq', 'cutoff'],
                'time': ['time', 'delay time', 'decay', 'decay time'],
                'feedback': ['feedback', 'regeneration', 'regen'],
                'amount': ['amount', 'depth', 'intensity'],
                'q': ['q', 'bandwidth', 'resonance'],
                'type': ['type', 'filter type', 'mode'],
            }

            target_setting = setting.lower()
            search_names = setting_aliases.get(target_setting, [target_setting])

            target_param_index = -1
            for i in range(min(param_count, 64)):  # Max 64 params to scan
                try:
                    param_name_result = await track_fx_get_param_name(track_index, target_fx_index, i)
                    param_name = param_name_result.split(": ", 1)[-1].lower() if ": " in param_name_result else param_name_result.lower()
                    for search in search_names:
                        if search in param_name or param_name in search:
                            target_param_index = i
                            break
            ...
            if target_param_index == -1:
                # If no exact match, try the first parameter that partially matches the setting name
                for i in range(min(param_count, 64)):
                    try:
                        param_name_result = await track_fx_get_param_name(track_index, target_fx_index, i)
                        param_name = param_name_result.split(": ", 1)[-1].lower() if ": " in param_name_result else param_name_result.lower()
                        if target_setting[:3] in param_name:
                            target_param_index = i
                            break
                    except:
                        continue
            ...
            if target_param_index == -1:
                return f"Parameter '{setting}' not found for {effect} on track {track_index + 1}"

            # Normalize value to 0-1 range if needed
            # Get current param range to determine if we need to scale
            try:
                param_info = await track_fx_get_param(track_index, target_fx_index, target_param_index)
                # Parse "Parameter X: value (range: min to max)"
                if "range:" in str(param_info):
                    range_part = str(param_info).split("range:")[1]
                    range_vals = range_part.strip().strip(")").split(" to ")
                    min_val = float(range_vals[0].strip())
                    max_val = float(range_vals[1].strip())

                    # If value is clearly in absolute range (e.g., frequency 200, gain -3)
                    if max_val > 1.0 and isinstance(value, (int, float)) and not isinstance(value, str):
                        if numeric_value > 1.0 or numeric_value < 0.0:
                            # Value is in absolute terms, normalize to 0-1
                            numeric_value = max(0.0, min(1.0, (numeric_value - min_val) / (max_val - min_val))) if max_val != min_val else 0.5
            except:
                pass

            # Ensure value is in 0-1 range
            final_value = max(0.0, min(1.0, float(numeric_value)))

            # Set the parameter
            set_result = await track_fx_set_param(track_index, target_fx_index, target_param_index, final_value)
    ...
    async def dsl_effect_bypass(
        ...
        try:
            from .resolvers import resolve_track
            from server.tools.fx import track_fx_set_enabled, track_fx_get_count, track_fx_get_name
            ...
            await track_fx_set_enabled(track_index, target_fx_index, not bypass)
    ...
    @mcp.tool()
    async def dsl_list_fx(
        track: Union[str, int, Dict[str, Any]]
    ) -> str:
        """
        List all effects on a track with their status. ALWAYS use this BEFORE adding new effects
        to check what's already there. Use for 'what effects are on this track', 'show FX chain',
        'list plugins', 'what's on the guitar'.

        Args:
            track: Track reference (name, index, or role)

        Examples:
            - "what effects does the guitar have?"
            - "show me the FX chain"
            - "list plugins on vocals"
            - "what's on track 1?"
        """
        try:
            from .resolvers import resolve_track
            from server.tools.fx import (
                track_fx_get_count, track_fx_get_name, track_fx_get_enabled
            )

            resolved_track = await resolve_track(bridge, track)
            track_index = resolved_track.index
            track_name = resolved_track.name or f"Track {track_index + 1}"
            ...
            if fx_count == 0:
                return f"Track '{track_name}' has no effects. You can add effects with dsl_add_effect."
            ...
            for i in range(fx_count):
                try:
                    name_result = await track_fx_get_name(track_index, i)
                    fx_name = name_result.split(": ", 1)[-1] if ": " in name_result else name_result

                    enabled_result = await track_fx_get_enabled(track_index, i)
                    is_enabled = "enabled" in enabled_result.lower() and "disabled" not in enabled_result.lower()

                    status = "ON" if is_enabled else "BYPASS"
                    fx_list.append(f"  {i+1}. {fx_name} [{status}]")
                except:
                    fx_list.append(f"  {i+1}. (could not read)")

            result = f"FX chain on '{track_name}' ({fx_count} effects):\n"
            result += "\n".join(fx_list)
            result += "\n\nTip: Use dsl_inspect_fx to see parameter details, or dsl_adjust_effect to change settings."

            return result

        except Exception as e:
            return f"Failed to list FX: {str(e)}"

    @mcp.tool()
    async def dsl_inspect_fx(
        track: Union[str, int, Dict[str, Any]],
        effect: str
    ) -> str:
        """
        Inspect a specific effect's parameters and current values. Use this to understand how an
        effect is currently configured before making changes. Shows parameter names and values.

        Args:
            track: Track reference (name, index, or role)
            effect: Effect name or partial name (e.g., "ReaEQ", "compressor", "reverb")

        Examples:
            - "show me the EQ settings"
            - "inspect the compressor on vocals"
            - "what are the reverb parameters?"
        """
        try:
            from .resolvers import resolve_track
            from server.tools.fx import (
                track_fx_get_count, track_fx_get_name, track_fx_get_param_count,
                track_fx_get_param_name, track_fx_get_param
            )
            ...
            try:
                parts = str(fx_count_str).split()
                fx_count = int(parts[parts.index("has") + 1]) if "has" in parts else 0
            except:
                fx_count = 0

            if fx_count == 0:
                return f"No effects on track '{track_name}'"
            ...
            for i in range(fx_count):
                try:
                    name_result = await track_fx_get_name(track_index, i)
                    fx_name_str = name_result.split(": ", 1)[-1] if ": " in name_result else name_result
                    if fx_search_name in fx_name_str.lower() or fx_name_str.lower() in fx_search_name:
                        target_fx_index = i
                        actual_fx_name = fx_name_str
                        break
                except:
                    continue

            if target_fx_index == -1:
                return f"Effect '{effect}' not found on track '{track_name}'"
            ...
            # Read all parameters (up to 50)
            params = []
            max_params = min(param_count, 50)
            failed_params = 0
            for i in range(max_params):
                try:
                    pname = await track_fx_get_param_name(track_index, target_fx_index, i)
                    pname = pname.split(": ", 1)[-1] if ": " in pname else pname

                    pval = await track_fx_get_param(track_index, target_fx_index, i)
                    # Parse "Parameter X: 0.500 (range: 0.000 to 1.000)"
                    val_str = pval.split(": ", 1)[-1] if ": " in pval else pval

                    params.append(f"  [{i}] {pname}: {val_str}")
                except Exception as e:
                    params.append(f"  [{i}] Parameter {i}: (error reading)")
                    failed_params += 1
                    continue

            result = f"Effect '{actual_fx_name}' on track '{track_name}' (FX slot {target_fx_index + 1}, {param_count} params):\n"
            if params:
                result += "\n".join(params)
            else:
                result += "  No readable parameters"

            if param_count > max_params:
                result += f"\n  ... and {param_count - max_params} more parameters (increase limit if needed)"

            result += "\n\nUse dsl_adjust_effect to change any of these settings."

            return result

        except Exception as e:
            return f"Failed to inspect FX: {str(e)}"

    @mcp.tool()
    async def dsl_get_track_info(
        track: Union[str, int, Dict[str, Any]]
    ) -> str:
        """
        Get complete information about a track including volume, pan, and all effects.
        Use this to understand the current state of a track before making changes.

        Args:
            track: Track reference (name, index, or role)

        Examples:
            - "tell me about the guitar track"
            - "what's the state of track 1?"
            - "show me the bass track info"
        """
        try:
            from .resolvers import resolve_track
            from server.tools.fx import track_fx_get_count, track_fx_get_name, track_fx_get_enabled

            resolved_track = await resolve_track(bridge, track)
            track_index = resolved_track.index
            track_name = resolved_track.name or f"Track {track_index + 1}"

            # Get volume and pan using GetMediaTrackInfo_Value (more reliable than GetTrackUIVol)
            vol_result = await bridge.call_lua("GetMediaTrackInfo_Value", [track_index, "D_VOL"])
            pan_result = await bridge.call_lua("GetMediaTrackInfo_Value", [track_index, "D_PAN"])
            mute_result = await bridge.call_lua("GetMediaTrackInfo_Value", [track_index, "B_MUTE"])
            solo_result = await bridge.call_lua("GetMediaTrackInfo_Value", [track_index, "I_SOLO"])

            volume_db = "N/A"
            if vol_result.get('ok'):
                vol = vol_result.get('ret', 1.0)
                ...
                if isinstance(vol, (int, float)) and vol > 0:
                    volume_db = f"{20 * math.log10(vol):.1f} dB"
                elif vol == 0:
                    volume_db = "-inf dB"

            pan_val = "N/A"
            if pan_result.get('ok'):
                p = pan_result.get('ret', 0.0)
                if isinstance(p, str):
                    try:
                        p = float(p)
                    except:
                ...
                if isinstance(p, (int, float)):
                    if p < -0.01:
                        pan_val = f"L{abs(int(p * 100))}"
                    elif p > 0.01:
                        pan_val = f"R{int(p * 100)}"
                    else:
                        pan_val = "Center"

            is_muted = mute_result.get('ret', 0) == 1 if mute_result.get('ok') else False
            is_soloed = solo_result.get('ret', 0) > 0 if solo_result.get('ok') else False
            ...
            # Build result
            result = f"Track '{track_name}' (index {track_index}):\n"
            result += f"  Volume: {volume_db}\n"
            result += f"  Pan: {pan_val}\n"
            result += f"  Muted: {'Yes' if is_muted else 'No'}\n"
            result += f"  Soloed: {'Yes' if is_soloed else 'No'}\n"
            result += f"  Effects ({fx_count}):\n"
            if fx_list:
                result += "\n".join(fx_list)
            else:
                result += "    (none)"
            ...
            return f"Failed to get track info: {str(e)}"

    # Count registered tools
    tool_count = 49  # Was 46, added 3 more tools (dsl_list_fx, dsl_inspect_fx, dsl_get_track_info)
